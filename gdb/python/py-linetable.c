...
build_linetable_entry (int line, CORE_ADDR address, unsigned int is_stmt = 0, bool prologue_end = false)
{
  ...
  if (obj != NULL)
    {
      obj->entry.line = line;
      obj->entry.set_unrelocated_pc (unrelocated_addr (address));
...
ltpy_entry_get_pc (PyObject *self, void *closure)
{
  ...
  return gdb_py_object_from_ulongest (CORE_ADDR (obj->entry.unrelocated_pc ())).release ();
}
...
ltpy_entry_init (PyObject *zelf, PyObject *args, PyObject *kw)
{
  linetable_entry_object *self = (linetable_entry_object *) zelf;
   ...
   self->entry.set_unrelocated_pc( unrelocated_addr (pc));
...
ltpy_iternext (PyObject *self)
{
  ...
  obj = build_linetable_entry (item->line, item->pc (objfile), item->is_stmt, item->prologue_end);
  iter_obj->current_index++;
